...
public class T {
    ...
    int getAnswer(String s) {
        ...
        for (char removedCh = 'z'; removedCh >= 'a'; --removedCh) {
            for (int direction = 0; direction < 2; ++direction) {
                ...
                for (char ch : s.toCharArray()) {
                    ...
                    if (ch == removedCh) {
                        if (builder.length() > 0) {
                            char builderLast = builder.charAt(builder.length() - 1);
                            if (builderLast == removedCh - 1) {
                                removed = true;
                            }
                        }
                    }

                    if (!removed) builder.append(ch);
                }
