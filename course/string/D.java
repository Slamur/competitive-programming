...
public class D {
    ...
    private void solve() {
        ...
        Point answer = getAnswer(s);
        out.println(answer.x + " " + answer.y);
    }

    Point getAnswer(String s) {
        String[] bracketParts = s.split("[()]");

        int maxLengthOutside = 0;
        int wordsCountInside = 0;

        for (int partIndex = 0; partIndex < bracketParts.length; ++partIndex) {
            boolean outside = (partIndex % 2 == 0);
            boolean inside = !outside;

            String part = bracketParts[partIndex];

            String[] words = part.split("_");
            for (String word : words) {
                if (word.isEmpty()) continue;

                if (inside) ++wordsCountInside;
                if (outside) {
                    maxLengthOutside = Math.max(
                            maxLengthOutside, word.length()
                    );
                }
            }
        }

        return new Point(maxLengthOutside, wordsCountInside);
    }
