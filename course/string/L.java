...
public class L {
    ...
    private void solve() {
        ...
        Point[] operations = getAnswer(s);

        String operationIds = "LR";

        out.println(operations.length);
        for (Point operation : operations) {
            out.println(operationIds.charAt(operation.x) + " " + operation.y);
        }
    }

    Point[] getAnswer(String s) {
        int n = s.length();
