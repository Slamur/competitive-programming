...
public class M {
    ...
    private void solve() {
        ...
        int typeIndex = getTypeIndex(number);

        String[] types = {
                "byte", "short", "int", "long", "BigInteger"
        };

        out.println(types[typeIndex]);
    }

    int getTypeIndex(String number){
        String[] limits = {
                "127", "32767", "2147483647", "9223372036854775807"
        };

        for (int typeIndex = 0; typeIndex < limits.length; ++typeIndex) {
            String limit = limits[typeIndex];

            boolean correct = (number.length() < limit.length());
            correct |= (number.length() == limit.length() && 0 <= limit.compareTo(number));
