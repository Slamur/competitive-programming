...
public class C implements Runnable{
    ...
    private void solve() {
        int maxDay = readInt();
        int k = readInt();
        int[][] strikeInfos = readIntMatrix(k, 2);

        int answer = getAnswer(maxDay, k, strikeInfos);
        out.println(answer);
    }

    int getAnswer(int maxDay, int k, int[][] strikeInfos) {
        boolean[] wasStrike = new boolean[maxDay + 1];
        for (int[] strikeInfo : strikeInfos) {
            int start = strikeInfo[0], step = strikeInfo[1];
            for (int day = start; day <= maxDay; day += step) {
                wasStrike[day] = true;
            }
        }

        for (int weekendDay = 6; weekendDay <= 7; ++weekendDay) {
            for (int day = weekendDay; day <= maxDay; day += 7) {
                wasStrike[day] = false;
            }
        }

        int answer = 0;
        for (boolean strike : wasStrike) {
            answer += strike ? 1 : 0;
        }
