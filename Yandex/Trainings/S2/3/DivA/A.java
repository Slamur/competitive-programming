...
public class A implements Runnable{
    ...
    private void solve() {
        ...
        for (int i = 1; i <= maxValue; ++i) answers.add(i);

        while (true) {
            String line = readLine();
            if ("HELP".equals(line)) break;

            String[] parts = line.split(" ");

            Set<Integer> values = Arrays.stream(parts)
                    .map(Integer::parseInt)
                    .filter(answers::contains)
                    .collect(Collectors.toSet());

            if (values.size() * 2 <= answers.size()) {
                answers.removeAll(values);
                out.println("NO");
            } else {
                answers.retainAll(values);
                out.println("YES");
            }
        }
