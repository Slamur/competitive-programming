...
public class A_SortedMap implements Runnable {
    ...
    private void solve() {
        int n = readInt();
        SortedMap<Long, Long> colorToSum = new TreeMap<>();

        for (int i = 0; i < n; ++i) {
            long color = readLong();
            long value = readLong();

            long oldSum = colorToSum.getOrDefault(color, 0L);
            colorToSum.put(color, oldSum + value);
        }

        for (Map.Entry<Long, Long> e : colorToSum.entrySet()) {
