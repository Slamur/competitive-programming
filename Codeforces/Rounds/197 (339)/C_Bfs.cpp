...
struct vertex {
	...
	vertex(int i, int d, int w) : i(i), d(d), w(w) { }
};
...
void solve_test() {
	...
	while (tests-- > 0) {
		...
		queue<vertex> q;
		vector<vvi> p(m + 1, vvi(k, vi(k, NONE)));
		...
		while (q.size()) {
			...
			for (int w = 0; w < k; ++w) {
				if (has[w] && d < w && prev != w) {
					...
					if (NONE != p[next_i][next_d][w]) continue;

					p[next_i][next_d][w] = prev;
					q.push({ next_i, next_d, w });
				}
			}
		}
