...
private:
	...
	void merge(int target, int source) {
		auto& v_target = tree[target];
		auto& v_source = tree[source];
		
		if (v_target.first > v_source.first) {
			v_target.first = v_source.first;
			v_target.second = 0;
		}
		
		if (v_target.first == v_source.first) {
			v_target.second += v_source.second;
		}
	}
...
public:
	...
	const T& get_min(int _start, int _end) { // [_start, _end]
		start = _start;
		end = _end + 1;
...
int main() {
	...
	while (q --> 0) {
		...
		if (SET == type) {
			...
			auto& [min, count] = tree.get_min(start, end - 1);
			cout << min << " " << count << "\n";
		}
	}
