...
ans_t get_answer(ll x, ll p, int k, int m, ll d) {
	int block_size = (p - 1) * m;

	vll powers(block_size + 1, 0);
	...
	vll prefs(block_size + 1, 1);
	for (int i = 1; i < prefs.size(); ++i) {
		prefs[i] = (prefs[i - 1] + powers[i]) % m;
	}

	ll block_cnt = 0;

	for (int i = 1; i < prefs.size(); ++i) {
		if (0 == prefs[i]) ++block_cnt;
	}
	...
	while (right - left > 1) {
		...
		for (int j = 1; j < k; ++j) {
			ll full_blocks = pos[j - 1] / block_size;
			pos[j] += block_cnt * full_blocks;

			ll delta = pos[j - 1] % block_size;
...
int main() {
	...
	for (int test = 1; test <= tests; ++test) {
		...
		ans_t answer = get_answer(x, p, k, m, d);
		cout << answer << ENDL;
		//print(answer);
	}
#else
