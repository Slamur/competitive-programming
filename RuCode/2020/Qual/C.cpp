...
fraction calc(fraction & first, fraction & second, int op) {
	switch (op)
	{
...
ans_t get_answer(vi & a) {
	...
	do {
		for (int op_mask = 0; op_mask < 64; ++op_mask) {
			for (int x = op_mask, i = 0; i < 3; ++i, x >>= 2) {
				ops[i] = x & 3;
			}
			...
			for (int fp = 0; fp < 3; ++fp) {
				for (int sp = 0; sp < 2; ++sp) {
					for (int tp = 0; tp < 1; ++tp) {
						vector<fraction> results(4, fraction());
						for (int i = 0; i < 4; ++i) {
							results[i] = fraction(a[i]);
						}

						results[fp] = calc(results[fp], results[fp + 1], ops[0]);
						results.erase(results.begin() + fp + 1);

						results[sp] = calc(results[sp], results[sp + 1], ops[1]);
						results.erase(results.begin() + sp + 1);

						results[tp] = calc(results[tp], results[tp + 1], ops[2]);
						results.erase(results.begin() + tp + 1);

						fraction result = results[0];
						if (result.is_int()) {
							fraction ans_d = (answer - goal);
							fraction d = (result - goal);

							if (ans_d.abs() > d.abs()) {
								answer = result;
							}
							else if (ans_d.abs() < d.abs()) {
								// none
							}
							else {
								if (answer > result) {
