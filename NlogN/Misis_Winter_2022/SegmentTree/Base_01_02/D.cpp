...
int main() {
	...
	for (int i = 0; i < q; ++i) {
		...
		ll score = scores[index];
		
		int start = index + 1;
		int end = index + score;
		
		ll suffix = (start <= n ? tree.get(start, min(end, n)) : 0);
		ll prefix = (end > n ? tree.get(1, end - n) : 0);
		
		scores[index] = (prefix + suffix) / score;
		tree.update(index, scores[index]);
	}
